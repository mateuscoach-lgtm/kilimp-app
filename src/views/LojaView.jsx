...
export default function LojaView({
  ...
  return (
    <div style={{ maxWidth: 520, margin: '0 auto', paddingBottom: totalItens > 0 ? 92 : 24 }}>
      <div style={{
        background: `linear-gradient(135deg, ${ACCENT} 0%, ${ACCENT_DARK} 100%)`,
        padding: '20px 18px 24px', color: '#fff', borderRadius: '0 0 22px 22px',
      ...
      <div style={{ padding: '10px 16px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        {produtos.map(prod => {
          ...
          return (
            <div key={prod.id} style={{ background: '#fff', borderRadius: 14, padding: 12, border: '1px solid #E3EAF3', display: 'flex', flexDirection: 'column', gap: 6 }}>
              <ProductThumb produto={prod} size={46} />
              <div style={{ fontSize: 13.5, fontWeight: 600, lineHeight: 1.25, minHeight: 34, marginTop: 4 }}>{prod.nome}</div>
              <div style={{ fontSize: 11.5, color: '#7C8B9C' }}>{prod.unidade}</div>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 4 }}>
                <div style={{ fontWeight: 700, fontSize: 15, color: ACCENT_DARK }}>{formatBRL(prod.preco)}</div>
                {qty === 0 ? (
      ...
      {totalItens > 0 && (
        <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, display: 'flex', justifyContent: 'center', padding: 12, background: 'linear-gradient(to top, rgba(245,248,252,1) 60%, rgba(245,248,252,0))' }}>
          <button onClick={irParaCarrinho} style={{ width: '100%', maxWidth: 488, background: ACCENT_DARK, color: '#fff', border: 'none', borderRadius: 14, padding: '14px 18px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', boxShadow: '0 6px 16px rgba(10,61,122,0.35)', fontSize: 14.5, fontWeight: 700 }}>
